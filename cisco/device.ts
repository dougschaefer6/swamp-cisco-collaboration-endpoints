 ...
 * fleet of registered endpoints with product type, workspace binding,
 * and connectivity state. Get returns full device detail including
 * software, network, and platform attributes. getStatus issues xAPI
 * status queries against named paths (e.g., `Audio.Microphones.*`,
 * `Conference.DoNotDisturb`) so workflows can poll specific
 * telemetry without retrieving the full device body. enableMacros
 * gates macro execution on a device — RoomOS ships with macros
 * disabled by default and requires explicit activation per endpoint.
 */
export const model = {
