...
/**
 * `@dougschaefer/cisco-collaboration-endpoints-macro` model — RoomOS
 * macro lifecycle management against a Cisco endpoint via Webex
 * Control Hub xAPI. List enumerates macros installed on a device.
 * Get returns a single macro's source. Save uploads or replaces a
 * macro and reports per-line deployment results, distinguishing
 * compile errors from runtime activation. Activate toggles a
 * macro's running state. Note: device.enableMacros must succeed
 * first or the macro runtime won't execute regardless of activation
 * state.
 */
export const model = {
